...
int gait[]={0,0,180,180}; // diagonally alternating

float cycle_period = 1; // in seconds... syncs legs with spine
float spine_timing_offset = cycle_period*(3.0/4.0); // T/4 for left front leg down & spine arching left, 3T/4 for left front leg down and spine arching right
...
float phi_s=50; //in deg, ϕ_s is the angular extent of the slow phase
float phi_0=60; //in deg, ϕ_0 is the center of the slow phase in degrees
float duty_cycle =0.75;//d_c is the duty cycle of the slow phase (i.e. fraction of the period spent in the slow phase). 
...
void setup() {
  ...
  for (int i=0; i<total_legs; i++){
      int reset_angle = (leg_offset[i]<1)? 360 + leg_offset[i]: leg_offset[i];
