...
%% Penetration Depth + Aerial Phase Per Experiment Consolidator — loads and analyzes all experiments in one shot
clear
clc
...
function run_combiner(cfg)
    ...
    % Uses the theta values from the quaternion analysis
    % Con: a > 0
    % Des: a < 0
    % Other: a = 0
    %
    ...
    for i = 1:nP
        ...
        theta_deg = quat.avg_yaw_theta(quat_idx); % theta from quaternion analysis
    ...
    if cfg.has_terrain % This is really for my data only

        Terrain_String = data.Terrain_String;

        % Apply aerial correction by phase:
        % Con = s + a
        % Des = s + a   (with a negative a)
        % Everything else = s
