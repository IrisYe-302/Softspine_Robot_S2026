...
for i = 1:nP
    for j = 1:nT
        % Build filename
        string   = "July25_" + Phase_String(i) + "_T" + Trial_String(j);
...
%}
start=[[383, 50, 625]; [525, 680, 245];[625, 500, 300]];

%disp(start)

%% Plot Start(run for checking)
figure
for i = 1:nP
    for j = 1:nT
...
rear_y_clipped  = cell(nP, nT);
rear_z_clipped  = cell(nP, nT);
front_x_clipped = cell(nP, nT);
front_y_clipped = cell(nP, nT);
front_z_clipped = cell(nP, nT);
t_clipped       = cell(nP, nT);
...
for i = 1:nP
    for j = 1:nT
        ...
        rear_x_clipped{i,j}  = squeeze(rear_x(i,j,  s:L));
        rear_y_clipped{i,j}  = squeeze(rear_y(i,j,  s:L));
        rear_z_clipped{i,j}  = squeeze(rear_z(i,j,  s:L));
        front_x_clipped{i,j} = squeeze(front_x(i,j, s:L));
        front_y_clipped{i,j} = squeeze(front_y(i,j, s:L));
        front_z_clipped{i,j} = squeeze(front_z(i,j, s:L));
        t_clipped{i,j}       = squeeze(t(i,j, s:L));

        % zero-reference each signal to its own start frame
...
rear_dist  = cell(nP, nT);
front_dist = cell(nP, nT);
...
for i = 1:nP
    for j = 1:nT
        ...
        hold on
        tt = t_clipped{i,j};
        plot(tt, rear_x_clipped{i,j}, 'r')
        plot(tt, rear_y_clipped{i,j}, 'g')
        plot(tt, rear_z_clipped{i,j}, 'b')
        plot(tt, rear_dist{i,j}, 'Color', [0.6 0.6 0.6], 'LineWidth', 1.5)
        title(Phase_String(i) + " " + Trial_String(j))
        ylabel('distance / m')
        xlabel('time / s')
        hold off
...
for i = 1:nP
    for j = 1:nT
        ...
        hold on
        tt = t_clipped{i,j};
        plot(tt, front_x_clipped{i,j}, 'r')
        plot(tt, front_y_clipped{i,j}, 'g')
        plot(tt, front_z_clipped{i,j}, 'b')
        plot(tt, front_dist{i,j}, 'Color', [0.6 0.6 0.6], 'LineWidth', 1.5)
        title(Phase_String(i) + " " + Trial_String(j))
        ylabel('distance / m')
        xlabel('time / s')
        hold off
...
for i = 1:nP
    ...
    for j = 1:nT
        ...
        rear_dist_smooth  = movmean(rear_dist{i,j},  smooth_window);
        front_dist_smooth = movmean(front_dist{i,j}, smooth_window);
...
for i = 1:nP
    ...
    for j = 1:nT
        ...
        dist_smooth = movmean(rear_dist{i,j}, smooth_window_y);
        deriv = diff(dist_smooth) * 120;
        [~, locs] = findpeaks(deriv, 'MinPeakDistance', 80, 'MinPeakProminence', 0.008);

        if length(locs) < 2, continue, end

        y_rear_smooth  = movmean(rear_y_clipped{i,j},  smooth_window_y);
        y_front_smooth = movmean(front_y_clipped{i,j}, smooth_window_y);
...
disp('Penetration depth and modeled step length per phase:')
for i = 1:nP
    if isnan(d_phase(i))
        disp(Phase_String(i) + ": no data")
        continue
    end
    disp(Phase_String(i) + ": d = " + round(d_phase(i),2) + " cm, modeled step length = " + round(s_phase(i),2) + " ± " + round(std_s_phase(i),2) + " cm")
end