%% Parse Data from CSVs (July2_Data — all phases, 2-body, sand terrain)
clear
clc
...
front_offset = 23;  % SpineRobot_Front position starts at column W (x=W, y=X, z=Y)
rear_offset  = 7;   % SpineRobot_Aft position starts at column G (x=G, y=H, z=I)

for i = 1:nP
    for j = 1:nT
        % Build filename
        string   = "July2_" + Phase_String(i) + "_T" + Trial_String(j);
        ...
        front_z(i, j, 1:L) = -M(:, 2+front_offset); % data is flipped due to calibration
...
% Convert to m to use same process as rest of scripts
rear_x  = rear_x/1000;
rear_y  =  rear_y/1000;
rear_z  =  rear_z/1000;
front_x =  front_x/1000;
front_y = front_y/1000;
front_z = front_z/1000;

%% Compute Velocities (forward direction is Z)
rear_Vz  = zeros(nP, nT, 25000);
front_Vz = zeros(nP, nT, 25000);

for i = 1:nP
    for j = 1:nT
        if ~file_loaded(i,j), continue, end
        for m = 1:Length(i,j)-1
            rear_Vz(i,j,m)  = rear_z(i,j,m+1)  - rear_z(i,j,m);
            front_Vz(i,j,m) = front_z(i,j,m+1) - front_z(i,j,m);
        end
    end
end
rear_Vz  = rear_Vz  * 240; % 240 Hz frame rate instead of 120 (thanks... downstairs Motive...)
front_Vz = front_Vz * 240;

%% Start-Detection Z (manual start input... data too messy)
%{
window    = 10;
...
for i = 1:nP
    for j = 1:nT
        ...
        for m = 10:Length(i,j)-window
            window_mean = mean(rear_Vz(i,j,m:m+window));
...
%}
start=[[1350, 1400, 1090]; [1450, 1315, 1700];[1430, 2320, 1090]];
...
for i = 1:nP
    for j = 1:nT
        ...
        for m = start(i,j)+end_window : Length(i,j)-end_window
            window_mean = mean(rear_Vz(i,j,m:m+end_window));
...
%}
end_frame = [[3460,3230,2910]; [3780,3650,3820]; [4020, 4500, 3250]];
...
for i = 1:nP
    for j = 1:nT
        ...
        plot(reshape(rear_Vz(i,j,1:Length(i,j)), [1 Length(i,j)]))
        hold on
...
sgtitle("Reinforced Spine Sand Start and End")
...
sgtitle("Reinforced Spine Sand Rear Displacement vs. Time")
...
sgtitle("Reinforced Spine Sand Front Displacement vs. Time")
lgd = legend('X', 'Y', 'Z', 'total dist', 'Orientation', 'horizontal');
lgd.Units = 'normalized';
lgd.Position = [0.4, 0.02, 0.2, 0.02];
...
for i = 1:nP
    ...
    for j = 1:nT
        ...
        rear_deriv  = diff(rear_dist_smooth)  * 240;
        front_deriv = diff(front_dist_smooth) * 240;
...
save("DataProcessing2_results.mat", "Phase_String", ...
    "avg_step_per_phase", "std_step_per_phase", ...
    "d_phase", "s_phase", "start", "end_frame")
disp(" ")
disp("Saved results to DataProcessing2_results.mat")